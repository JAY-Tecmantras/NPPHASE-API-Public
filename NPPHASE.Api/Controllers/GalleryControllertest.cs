...
namespace NPPHASE.Apis.Controllers
{
    ...
    public class GalleryControllertest : BaseController<Gallery>
    {
        private readonly IActionResult _result;
        private readonly IService<Gallery> _service;
        private readonly IGalleryService _galleryService;

        public GalleryControllertest(IService<Gallery> service, IGalleryService galleryService, IDeviceUserServices deviceUserServices) : base(service, deviceUserServices)
        {
            _service = service;
        ...
        [HttpPost("DownloadZip")]
        public async Task<IActionResult> test([FromBody] List<int> galleryId)
        {
            if (galleryId?.Count == 0)
                return BadRequest(new ResponseMessageViewModel { IsSuccess = false, Message = "No gallery ids provided" });
